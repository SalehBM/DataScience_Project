...
def create(submission: SubmissionCreate, db: Session = DB):
    submission_db = Submission(
        ...
        speciality = submission.speciality,
...
def get_by_scores(id: int, db: Session = DB):
    return db.query(Submission).filter(Submission.course_id==id).order_by(Submission.score, Submission.date).all()

def get_approved_by_course_id(id: int, db: Session = DB):
    return db.query(Submission).filter(Submission.course_id==id, Submission.Approved == True).all()
