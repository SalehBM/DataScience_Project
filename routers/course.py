...
@router.get('/get_by_member{id}', status_code=status.HTTP_200_OK)
def get_by_member(id: int, auth: AdminUser = Depends(oauth2.get_current_user)):
    return courseDB.get_all_by_member(id)

@router.get('/getAllIncoming', status_code=status.HTTP_200_OK)
def get_all_incoming_courses(auth: AdminUser = Depends(oauth2.get_current_user)):
    return courseDB.get_all_incoming()
